...
namespace NzbDrone.Core.Download.Clients.QBittorrent
{
    public class QBittorrentSettingsValidator : AbstractValidator<QBittorrentSettings>
    {
        public QBittorrentSettingsValidator()
        {
            ...
            RuleFor(c => c.ApiKey).Empty().When(c => c.Username.IsNotNullOrWhiteSpace() || c.Password.IsNotNullOrWhiteSpace())
                .WithMessage("API Key cannot be used together with Username/Password. Clear one before saving.");

            RuleFor(c => c.Username).Empty().When(c => c.ApiKey.IsNotNullOrWhiteSpace())
                .WithMessage("Username cannot be set when using API Key.");

            RuleFor(c => c.Password).Empty().When(c => c.ApiKey.IsNotNullOrWhiteSpace())
                .WithMessage("Password cannot be set when using API Key.");
        }
    }
    ...
    public class QBittorrentSettings : IProviderConfig
    {
        ...
        [FieldDefinition(4, Label = "API Key", Type = FieldType.Textbox, Privacy = PrivacyLevel.ApiKey, HelpText = "qBittorrent 5.x+ API key. Use this instead of Username/Password.")]
